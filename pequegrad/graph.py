...
def get_tensor_repr(node: Tensor):
    ...
    if not hasattr(node, 'id'):
        node.id = ID
        ID += 1
    s = "shape: " + str(node.shape) if not is_scalar(node) else "value: " + str(node.numpy())
    return str(node.id) + "\n" + s
    ...
    while stack:
        ...
        if node not in visited:
            ...
            for c in node._ctx.children if node._ctx else []:
                stack.append(c)
                G.add_edge(get_tensor_repr(c), get_tensor_repr(node), label=node._ctx.__class__.__name__)
    return G
