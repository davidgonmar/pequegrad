    ...
    def backward(self):
        ...
        if self.x.requires_grad:
            if self.x.dim == 1 and self.y.dim == 1:
                ...
                self.x._grad += Tensor(grad_output * self.y.data, backend=self.backend)
            elif self.x.dim == 1:
                # Vector x Matrix
                self.x._grad += Tensor(
                    grad_output @ self.y.data.T, backend=self.backend
                )
            elif self.y.dim == 1:
                # Matrix x Vector
        ...
        if self.y.requires_grad:
            if self.x.dim == 1 and self.y.dim == 1:
                self.y._grad += Tensor(self.x.data * grad_output, backend=self.backend)
            elif self.x.dim == 1:
                self.y._grad += Tensor(
                    self.x.data.outer_product(grad_output), backend=self.backend
                )
            elif self.y.dim == 1:
                # Matrix x Vector
                self.y._grad += Tensor(
                    self.x.data.T @ grad_output, backend=self.backend
                )
            else:
                # Matrix x Matrix
