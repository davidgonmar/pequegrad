...
void launch_reduce_kernel(ReduceKernelType type, DType dtype, dim3 blocks,
  ...
  if (dtype == DType::Float32) {
    __launch_reduce_kernel<float>(type, blocks, threads,
                                  (const float *)in, (float *)out, in_strides,
                                  in_shape, n_dims, red_axis);
  } else if (dtype == DType::Int32) {
    __launch_reduce_kernel<int>(type, blocks, threads, (const int *)in,
                                (int *)out, in_strides, in_shape, n_dims,
                                red_axis);
  } else if (dtype == DType::Float64) {
    __launch_reduce_kernel<double>(type, blocks, threads,
                                   (const double *)in, (double *)out, in_strides,
                                   in_shape, n_dims, red_axis);
  }
}