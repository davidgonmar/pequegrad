...
template <typename T>
void __launch_binary_kernel(BinaryKernelType kernel_type, dim3 grid_size, dim3 block_size, const size_t *lhs_strides,
                            const size_t *rhs_strides, const size_t *shape,
                            const size_t num_dims, const T *lhs, const T *rhs,
                            T *out) {
    switch (kernel_type) {
        case BinaryKernelType::ADD:
            add_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::SUB:
            sub_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::MULT:
            mult_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::DIV:
            div_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::GREATER:
            greater_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::LESS:
            less_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::EQUAL:
            equal_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::NOT_EQUAL:
            not_equal_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::GREATER_EQUAL:
            greater_equal_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::LESS_EQUAL:
            less_equal_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::ELEMENT_WISE_MAX:
            element_wise_max_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
        case BinaryKernelType::POW:
            pow_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape, num_dims, lhs, rhs, out);
            break;
    }
}


void launch_binary_kernel(BinaryKernelType kernel_type, DType dtype, dim3 grid_size, dim3 block_size, const size_t *lhs_strides,
                          const size_t *rhs_strides, const size_t *shape,
