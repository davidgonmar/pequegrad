...
Device_Slice convert_to_slice(const slice_item_t &item) {
  ...
  if (std::holds_alternative<SliceFromSSS>(item)) {
    ...
    cudaMemcpy(slice.indices, idxArray.indices.data(),
               slice.indexSize * sizeof(int), cudaMemcpyHostToDevice);
...
CudaTensor _slice_with_array(const CudaTensor &ten, const slice_t &_slices) {
  ...
  case DType::Float32:
    _slice_and_assign_with_array_kernel<float><<<grid_size, block_size>>>(
        (float *)ten.get_base_ptr(), (float *)out.get_base_ptr(), d_shape.get(),
        out_d_shape.get(), src_strides.get(), ten.ndim(), d_slices.get(),
        slices.size(), false);
    break;
  case DType::Int32:
    _slice_and_assign_with_array_kernel<int><<<grid_size, block_size>>>(
        (int *)ten.get_base_ptr(), (int *)out.get_base_ptr(), d_shape.get(),
        out_d_shape.get(), src_strides.get(), ten.ndim(), d_slices.get(),
        slices.size(), false);
    break;
  case DType::Float64:
    _slice_and_assign_with_array_kernel<double><<<grid_size, block_size>>>(
        (double *)ten.get_base_ptr(), (double *)out.get_base_ptr(),
        d_shape.get(), out_d_shape.get(), src_strides.get(), ten.ndim(),
        d_slices.get(), slices.size(), false);
    break;
  }
...
CudaTensor _assign_with_array(const CudaTensor &ten, const slice_t &_slices,
                              const CudaTensor &_vals) {
  ...
  case DType::Float32:
    _slice_and_assign_with_array_kernel<float><<<grid_size, block_size>>>(
        (float *)ten.get_base_ptr(), (float *)vals.get_base_ptr(),
        d_shape.get(), out_d_shape.get(), src_strides.get(), ten.ndim(),
        d_slices.get(), slices.size(), true);
    break;
  case DType::Int32:
    _slice_and_assign_with_array_kernel<int><<<grid_size, block_size>>>(
        (int *)ten.get_base_ptr(), (int *)vals.get_base_ptr(), d_shape.get(),
        out_d_shape.get(), src_strides.get(), ten.ndim(), d_slices.get(),
        slices.size(), true);
    break;
  case DType::Float64:
    _slice_and_assign_with_array_kernel<double><<<grid_size, block_size>>>(
        (double *)ten.get_base_ptr(), (double *)vals.get_base_ptr(),
        d_shape.get(), out_d_shape.get(), src_strides.get(), ten.ndim(),
        d_slices.get(), slices.size(), true);
    break;
  }
...
CudaTensor CudaTensor::assign(const slice_t &slices, const CudaTensor &vals) {
  ...
  const CudaTensor _vals = vals.squeeze(squeeze_dims)
  ...
  auto &sliced_shape =
      cuda_unique_ptr_from_host(_sliced.shape.size(), _sliced.shape.data());
  auto &vals_shape =
  ...
  launch_copy_with_out_strides_kernel(
      ...
      sliced_strides.get(), sliced_shape.get(), _vals.ndim(), _sliced.ndim(),
      _vals.get_base_ptr(), _sliced.get_base_ptr());
