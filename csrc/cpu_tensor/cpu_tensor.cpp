...
CpuTensor CpuTensor::add(const CpuTensor &other) const {
  ...
  // Vector + Vector case
  if (shape.size() == 1) {
    add_vectorized(ptr1, ptr2, result, strides[0] / sizeof(float), other.strides[0] / sizeof(float), shape[0]);
  } else {
    ...
    for (int i = 0; i < total_vectors; i++) {
      add_vectorized(ptr1 + i * vecsize, ptr2 + i * vecsize, result + i * vecsize, strides[shape.size() - 1] / sizeof(float), other.strides[shape.size() - 1] / sizeof(float), vecsize);
    }
  }
