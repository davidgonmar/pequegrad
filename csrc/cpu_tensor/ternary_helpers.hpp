...
template <typename T> using Op = std::function<T(T, T, T)>;
...
static inline void _dispatch_teranry_op_helper(
    const shape_t &shape, const shape_t &a_strides, const shape_t &b_strides,
    const shape_t &c_strides, const shape_t &result_strides, const void *a,
    const void *b, const void *c, void *result, TernaryOpType op) {
  switch (op) {
...
static inline void dispatch_ternary_op(
    const shape_t &shape, const shape_t &a_strides, const shape_t &b_strides,
    const shape_t &c_strides, const shape_t &result_strides, const void *a,
    const void *b, const void *c, void *result, DType dtype, TernaryOpType op) {
  switch (dtype) {
  case DType::Float32:
