...
__global__ void NAME(\
    ...
    const size_t *shape,       /* both lhs and rhs should have equal shape, we don't handle broadcasting here */\
...
void launch_binary_kernel_casted_helper(BinaryKernelType kernel_type,
  ...
  case BinaryKernelType::GREATER_EQUAL:
    greater_equal_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape,
                                                     num_dims, lhs, rhs, out);
    break;
  case BinaryKernelType::LESS_EQUAL:
    less_equal_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape,
                                                  num_dims, lhs, rhs, out);
    break;
  case BinaryKernelType::ELEMENT_WISE_MAX:
    element_wise_max_kernel<<<grid_size, block_size>>>(lhs_strides, rhs_strides, shape,
                                                       num_dims, lhs, rhs, out);
    break;
