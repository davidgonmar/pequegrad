...
void __launch_ternary_kernel(TernaryKernelType kernel_type, dim3 grid_size,
                           dim3 block_size,
                           const size_t *first_strides,
                           const size_t *second_strides,
                           const size_t *third_strides, const size_t *shape,
                           const size_t num_dims, const T *first, const T *second,
                           const T *third, T *out) {
  switch (kernel_type) {
  case TernaryKernelType::WHERE:
    where_kernel<<<grid_size, block_size>>>(
        first_strides, second_strides, third_strides, shape, num_dims, first,
        second, third, out);
    break;
  }
}
