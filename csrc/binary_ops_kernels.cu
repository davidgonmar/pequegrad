...
void launch_binary_kernel(BinaryKernelType kernel_type, DType dtype, dim3 grid_size, dim3 block_size, const size_t *lhs_strides,
    ...
    switch (dtype) {
        case DType::Float32:
            __launch_binary_kernel<float>(kernel_type, grid_size, block_size, lhs_strides, rhs_strides, shape, num_dims,
                                         (const float *)lhs, (const float *)rhs, (float *)out);
            break;
        case DType::Float64:
            __launch_binary_kernel<double>(kernel_type, grid_size, block_size, lhs_strides, rhs_strides, shape, num_dims,
                                         (const double *)lhs, (const double *)rhs, (double *)out);
            break;
        case DType::Int32:
            __launch_binary_kernel<int>(kernel_type, grid_size, block_size, lhs_strides, rhs_strides, shape, num_dims,
                                         (const int *)lhs, (const int *)rhs, (int *)out);
            break;
    }
}