...
void Im2Col::dispatch_cpu(const std::vector<Tensor> &inputs,
  ...
  cpu::dispatch_im2col_kernel(
      a.dtype(), a.get_base_ptr(), outputs[0].get_base_ptr(), k_h, k_w, x_h,
      x_w, stride_x, stride_y, batch_size, in_channels, dilation_x, dilation_y);
}
...
void Col2Im::dispatch_cpu(const std::vector<Tensor> &inputs,
  ...
  cpu::dispatch_col2im_kernel(a.dtype(), a.get_base_ptr(),
