#define DEF_TERNARY_OP_KERNEL(KERNEL_NAME, FN)                                     \
  extern "C" __global__ void KERNEL_NAME(                                      \
    ...
    int idxF = 0;                                                            \
    int idxS = 0;  int idxT = 0;                                                           \
    int tmpI = idx;                                                            \
    for (int d = num_dims - 1; d >= 0; d--) {                                  \
      int dim = tmpI % shape[d];                                               \
      idxF +=                                                                \
          (dim * first_strides[d]); /* Convert byte stride to element stride */  \
      idxS +=                                                                \
          (dim * second_strides[d]); /* Convert byte stride to element stride */  \
    idxT +=                                                                \
          (dim * third_strides[d]); /* Convert byte stride to element stride */  \
      tmpI /= shape[d];                                                        \
    }                                                                          \
